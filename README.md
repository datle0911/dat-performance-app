...
6. Next, create a Container Registry in Azure to store the image, and AKS can later pull them during deployment to the AKS cluster. Feel free to change the ACR name in place of `myvoteappacr2024` below.
      ```bash
      # Assuming the acdnd-c4-project resource group is still avaiable with you
      # Create a resource group
      az group create --name acdnd-c4-project --location westus2
      # ACR name should not have upper case letter
      az acr create --resource-group acdnd-c4-project --name myvoteappacr2024 --sku Basic
      # Log in to the ACR
      az acr login --name myvoteappacr2024
      ...
      az acr show --name myvoteappacr2024 --query loginServer --output table
      # Associate a tag to the local image. You can use a different tag (say v2, v3, v4, ....) everytime you edit the underlying image. 
      docker tag azure-vote-front:v1 myvoteappacr2024.azurecr.io/azure-vote-front:v1
      # Now you will see myvoteappacr2024.azurecr.io/azure-vote-front:v1 if you run docker images
      # Push the local registry to remote ACR
      docker push myvoteappacr2024.azurecr.io/azure-vote-front:v1
      # Verify if you image is up in the cloud.
      az acr repository list --name myvoteappacr2024 --output table
      # Associate the AKS cluster with the ACR repository
      az aks update -n udacity-cluster -g acdnd-c4-project --attach-acr myvoteappacr2024
      ```
...
7. Now, deploy the images to the AKS cluster:
      ...
      # Get the ACR login server name
      az acr show --name myvoteappacr2024 --query loginServer --output table
      # Make sure that the manifest file *azure-vote-all-in-one-redis.yaml*, has `myvoteappacr2024.azurecr.io/azure-vote-front:v1` as the image path.  
      ...
      kubectl set image deployment azure-vote-front azure-vote-front=myvoteappacr2024.azurecr.io/azure-vote-front:v1      
